...
def does_collide(vertices1, vertices2) -> bool:
...
def cross(a1, a2, b1, b2):
    ...
    if t is not None and t >= 0 and t <= 1 and u is not None and u >= 0 and u <= 1:
        return True
...
def get_vertices(x, y, width, height, rot):
    ...
    return [
        ...
        bottomLeft,
        topLeft,
    ]  # making points to be all around
...
class BaseCar:
    ...
    def scale_car(self, val) -> None:  # Runs only when score is changed
        # math.Floor(x/y) same as x#y
        for _i in range(math.floor(val / 5)):
            # More score = Taller car
            self.body_height_scale = lerp(
                self.body_height_scale, BotCar.MAX_BODY_SCALE, 0.001
            )
            ...
            self.bumper_width_scale = lerp(
                self.bumper_width_scale, BotCar.MAX_BUMPER_SCALE * 1.5, 0.001
            )
    ...
    def get_bumper_vertices(self):
        x = (
            ...
            - math.sin(self.heartbeat_info["rot"]) * self.body_height / 2
        )
        y = (
            self.heartbeat_info["y"]
            + math.cos(self.heartbeat_info["rot"]) * self.body_height / 2
        )
    ...
    def add_score_for_killing(self, dead_car) -> None:
    ...
    def add_score_for_eating_food(self) -> None:  # Add score for eating food
...
class BotCar(BaseCar):
    ...
    def __init__(self, x, y, angle, color, name, score) -> None:
        super().__init__()
    ...
    def add_close_car(self, the_id, car_or_bot) -> None:
    ...
    def move(self, delta) -> None:
        ...
        self.xVel += (
            math.cos(self.heartbeat_info["rot"] + radians(90))
            * self.accSpeed
            * -self.heartbeat_info["acc"]
        )
        self.yVel += (
            math.sin(self.heartbeat_info["rot"] + radians(90))
            * self.accSpeed
            * -self.heartbeat_info["acc"]
        )  # acc => dopredu|dozadu
        self.heartbeat_info["rot"] += self.driftAngle * self.heartbeat_info["acc"]

        self.heartbeat_info["x"] = constrain(
            self.heartbeat_info["x"] + self.xVel, -MAP_SIDE, MAP_SIDE
        )
        self.heartbeat_info["y"] = constrain(
            self.heartbeat_info["y"] + self.yVel, -MAP_SIDE, MAP_SIDE
        )
    ...
    def does_eat(self, food) -> bool:  # Is food center isnide body/bumper?
        ...
        for body_or_bumper_vertices in len(body_bumper_vertices):
            ...
            if (
                scalABAF >= 0
                and scalABAF <= scalABAB
                and scalBCBF >= 0
                and scalBCBF <= scalBCBC
            ):
                return True
...
class PlayerCar(BaseCar):
    def __init__(self) -> None:
        ...
        self.heartbeat_info = {}
        self.heartbeat_info["x"] = (
            0  # Will be in middle - will see realtime action before he hops in
        )
    ...
    def spawn(self, name) -> None:
        ...
        self.heartbeat_info["name"] = (
            name if name != "" else "Smasher.ml"
